...
class IsinForward {
    ...
    void init(double radius, double centerLon, double falseEasting, double falseNorth, double dzone, double djustify) {
        ...
        if (Math.abs(dzone - nzone) > Const.EPS_CNVT) {
        ...
        if (djustify < -Const.EPS_CNVT || djustify > (2.0 + Const.EPS_CNVT)) {
            throw new IllegalArgumentException("bad parameter; djustify out of range");
        }

        final int ijustify = (int) (djustify + Const.EPS_CNVT);
        if (Math.abs(djustify - ijustify) > Const.EPS_CNVT) {
            throw new IllegalArgumentException("bad parameter; ijustify not near an integer value");
        }

        if (radius < EPS_SPHERE) {
            throw new IllegalArgumentException("bad parameter; sphere radius too small");
        }

        if (centerLon < -TWOPI || centerLon > TWOPI) {
            throw new IllegalArgumentException("bad parameter; center longitude invalid");
        }

        if (nzone < 2 || nzone > NROW_MAX) {
            throw new IllegalArgumentException("bad parameter; number of zones out of range");
        }
