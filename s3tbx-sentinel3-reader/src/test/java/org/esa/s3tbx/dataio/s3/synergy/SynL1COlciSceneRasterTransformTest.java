 ...
 * @author Tonio Fincke
 */
public class SynL1COlciSceneRasterTransformTest {

    private SynL1COlciSceneRasterTransform transform;
    ...
    public void setUp() {
        ...
        transform = new SynL1COlciSceneRasterTransform(colMisregistrationBand, rowMisregistrationBand);
    }

    @Test
    public void testGetForward() throws Exception {
        PixelPos resultPos = new PixelPos();
        transform.getForward().transform(new PixelPos(0, 0), resultPos);
        assertEquals(1, (int)resultPos.getX());
        assertEquals(-1, (int)resultPos.getY());

        transform.getForward().transform(new PixelPos(2, 2), resultPos);
        assertEquals(3, (int)resultPos.getX());
        assertEquals(1, (int)resultPos.getY());

        transform.getForward().transform(new PixelPos(5, 7), resultPos);
        assertEquals(7, (int)resultPos.getX());
        assertEquals(5, (int)resultPos.getY());

        transform.getForward().transform(new PixelPos(13, 11), resultPos);
        assertEquals(15, (int)resultPos.getX());
        assertEquals(9, (int)resultPos.getY());

        transform.getForward().transform(new PixelPos(15, 15), resultPos);
